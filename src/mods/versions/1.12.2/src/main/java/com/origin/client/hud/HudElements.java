...
public final class HudElements {
    ...
    public static void renderAll(ScaledResolution sr) {
        ...
        if (mc.player == null || mc.gameSettings.hideGUI || editorOpen) return;
    ...
    private static FontRenderer font() { return Minecraft.getMinecraft().fontRenderer; }
    ...
    private static final class CoordsElement extends Element {
        ...
        private List<String> lines(boolean preview) {
            ...
            if (mc.player != null) {
                px = mc.player.posX; py = mc.player.posY; pz = mc.player.posZ;
                facing = mc.player.getHorizontalFacing().toString();
                facing = Character.toUpperCase(facing.charAt(0)) + facing.substring(1);
                try {
                    biome = mc.world.getBiome(new BlockPos(mc.player)).getBiomeName();
    ...
    private static final class PotionHudElement extends Element {
        ...
        private List<PotionEffect> effects(boolean preview) {
            ...
            if (preview && (mc.player == null || mc.player.getActivePotionEffects().isEmpty())) {
                out.add(new PotionEffect(MobEffects.SPEED, 83 * 20, 0));
                out.add(new PotionEffect(MobEffects.STRENGTH, 45 * 20, 1));
                return out;
            }
            if (mc.player != null) {
        ...
        public void draw(int x, int y, boolean preview) {
            ...
            for (PotionEffect e : fx) {
                ...
                if (visible && potion.hasStatusIcon()) {
                    // GuiContainer.INVENTORY_BACKGROUND is protected — same texture.
    ...
    private static final class ArmorHudElement extends Element {
        ...
        private List<ItemStack> stacks(boolean preview) {
            ...
            if (mc.player != null) {
                for (int i = 3; i >= 0; i--) {
                    ItemStack st = mc.player.inventory.armorInventory.get(i);
                    out.add(st);
                    if (!st.isEmpty()) any = true;
                }
                ItemStack held = mc.player.getHeldItemMainhand();
                out.add(held);
                if (!held.isEmpty()) any = true;
            }
            if (preview && !any) {
                out.clear();
    ...
    private static final class ServerAddressElement extends Element {
        ...
        private String line(boolean preview) {
            ...
            try {
                int players = mc.getConnection() != null ? mc.getConnection().getPlayerInfoMap().size() : 0;
    ...
    private static final class SprintStateElement extends Element {
        ...
        private String line() {
            ...
            if (Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.isSprinting())
                return "Sprinting";
        ...
        public void draw(int x, int y, boolean preview) {
            boolean active = preview || ToggleSprint.sprintToggled || ToggleSprint.sneakToggled
                || (Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.isSprinting());
            if (!active) return;
            int[] s = size(preview);
            backing(x, y, s[0], s[1]);
