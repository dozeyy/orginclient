...
public final class OriginScreens {
    ...
    public void onInitGui(GuiScreenEvent.InitGuiEvent.Post event) {
        buttons.put(event.getGui(), event.getButtonList());
        if (event.getGui() instanceof GuiMainMenu && originStyleActive()) {
            ...
            for (GuiButton b : event.getButtonList()) {
                ...
                if (b.visible && b.id != 14) {
                    fullX = Math.min(fullX, b.x);
                    fullRight = Math.max(fullRight, b.x + b.width);
                }
            }
    ...
    public void onDrawPre(GuiScreenEvent.DrawScreenEvent.Pre event) {
        ...
        try {
            if (gui instanceof GuiMainMenu) {
                if (drawTitle((GuiMainMenu) gui, event.getMouseX(), event.getMouseY())) event.setCanceled(true);
            } else if (gui instanceof GuiConnecting) {
                if (drawConnecting(gui, event.getMouseX(), event.getMouseY())) event.setCanceled(true);
    ...
    public void onDrawPost(GuiScreenEvent.DrawScreenEvent.Post event) {
        ...
        OriginScreenRenderer.renderCursorGlow(gui.width, event.getMouseX(), event.getMouseY(), false);
    }
    ...
    private void drawOriginButton(GuiButton b, int mouseX, int mouseY) {
        ...
        OriginUi.button(b.x, b.y, b.width, b.height, t, b.enabled);
        String label = cleanLabel(b.displayString);
        OriginUi.label(label, b.x + b.width / 2.0, b.y + b.height / 2.0,
    ...
    private void drawAccountChip(int w) {
        ...
        int chipW = padX + head + gap + mc.fontRenderer.getStringWidth(name) + padX;
        ...
        mc.fontRenderer.drawString(name, x + padX + head + gap, y + (chipH - 8) / 2, OriginTheme.TEXT, false);
    }
