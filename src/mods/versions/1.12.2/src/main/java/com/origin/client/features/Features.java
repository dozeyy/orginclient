...
public final class Features {
    ...
    private void tickFullbright() {
        if (Mods.isOn("fullbright")) {
            ...
            if (mc.player != null && existing == null) {
                mc.player.addPotionEffect(new net.minecraft.potion.PotionEffect(
                    net.minecraft.init.MobEffects.NIGHT_VISION, 1000000, 0, true, false));
    ...
    private net.minecraft.world.WorldServer serverWorld(WorldClient client) {
        try {
            return mc.getIntegratedServer().getWorld(client.provider.getDimension());
        } catch (Throwable t) {
            return null;
        }
    }
    ...
    private void setServerDaylight(boolean on) {
        try {
            net.minecraft.world.WorldServer sw = serverWorld(mc.world);
            if (sw != null)
