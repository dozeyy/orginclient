...
public final class BlockOverlayRenderer {
	...
	public static boolean onBlockOutline(WorldRenderContext wctx, BlockOutlineRenderState octx) {
		...
		if (outline && !overlay) {
			...
			// little inward dots at the corners. It also grew OUTWARD, floating
			// the outline off the block as a cage.
			//
			// Now each edge is one solid box (ThickLine), sized by the slider and
			// biased inward so its outer face stays on the block surface.
			//
			// The centre comes from the SHAPE's bounds, not the block's: any shape
			// not straddling 0.5 (slab, stairs, fence, pane) would otherwise have
			// both endpoints pushed the same way, TRANSLATING the outline off the
