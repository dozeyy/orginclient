...
public final class HudElements {
	...
	private static List<String> coordsLines(Minecraft mc) {
		...
		if (Mods.bool("coords", "biome") && mc.level != null) {
			...
			// added in 1.19.4 and this jar's floor is 1.19.3 (same block pos here).
	...
	// MobEffectInstance.isInfiniteDuration() only exists since 1.19.4; this
	// jar's range floor is 1.19.3, where effects simply can't be infinite. The
	// latch turns the one-time NoSuchMethodError into a permanent false instead
	// of throwing out of measure() every frame.
	private static volatile boolean infiniteApiBroken = false;

	private static boolean infiniteDuration(MobEffectInstance e) {
		if (infiniteApiBroken) {
			return false;
		}
		try {
			return e.isInfiniteDuration();
		} catch (Throwable t) {
			infiniteApiBroken = true;
			return false;
		}
	}
