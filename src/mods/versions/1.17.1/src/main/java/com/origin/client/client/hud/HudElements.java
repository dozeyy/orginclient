...
public final class HudElements {
	...
	private static List<String> coordsLines(Minecraft mc) {
		...
		if (Mods.bool("coords", "biome") && mc.level != null) {
			...
			// added in 1.19.4 (same block pos here).
			// 1.17.1: getBiome returns a bare Biome (no Holder/unwrapKey — those
			// are 1.18.2+); resolve the key through the biome registry instead.
			var biome = mc.level.getBiome(pl.blockPosition());
