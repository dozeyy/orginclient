...
public final class BlockOverlayRenderer {
	...
	public static boolean onBlockOutline(WorldRenderContext wctx, WorldRenderContext.BlockOutlineContext octx) {
		...
		if (overlay) {
			...
			// 1.18.2 has no RenderType.debugQuads() (1.19.4+) and no other
			...
			// 1.18.2 upload: end() is void; BufferUploader.end(q) draws with the
			// shader set above (drawWithShader/RenderedBuffer are 1.19+).
			q.end();
			BufferUploader.end(q);
