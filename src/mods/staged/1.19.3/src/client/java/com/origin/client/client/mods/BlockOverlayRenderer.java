...
public final class BlockOverlayRenderer {
	...
	// package-private RenderType.create is unreachable, so the overlay's fill
	// type is built on the public RenderType constructor reusing vanilla's OWN
	// state shards -- POSITION_COLOR_SHADER + TRANSLUCENT_TRANSPARENCY +
	// NO_CULL, the exact three debug_quads composes on 1.19.4
	// (bytecode-verified; buffer size 131072, no crumbling, no sort-on-upload,
	// same as 1.19.4's 5-arg create defaults). Setup/clear delegate to the
	// shards themselves so the GL state is vanilla's, not a re-implementation.
	private static final class FillQuads extends RenderType {
		static final RenderType INSTANCE = new FillQuads();
